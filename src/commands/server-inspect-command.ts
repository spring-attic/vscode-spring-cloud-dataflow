...
import { COMMAND_SCDF_SERVER_INSPECT } from '../extension-globals';
...
@injectable()
export class ServerInspectCommand implements Command {
    ...
    get id() {
        return COMMAND_SCDF_SERVER_INSPECT;
    }

    async execute(serverNode: ServerNode) {
        const server: ServerRegistration = serverNode.registration;
        const model = new ScdfModel(server);
        const data = await model.about();
        const node = {label: `${serverNode.label}`, fullId: serverNode.label};
